...
class BrowserAutomation:
    ...
    def _detect_login_state(self, page):
        try:
            ...
            login_words = ['log in', 'login', 'sign in', 'đăng nhập']
            if any(x in body for x in login_words):
                return 'login_required'
            # A loaded page with meaningful content and no sign-in controls is a stronger signal than URL alone.
        ...
        try:
            ...
            with sync_playwright() as pw:
                context = pw.chromium.launch_persistent_context(
                    str(META_PROFILE), headless=False, accept_downloads=True,
...
class FadePage(QWidget):
    ...
    def fade_in(self):
        ...
        anim.setDuration({'Nhẹ': 140, 'Vừa': 220, 'Mạnh': 330}.get(self.level, 220))
...
class MainWindow(QMainWindow):
    ...
    def show_page(self,index):
        ...
        for i,b in enumerate(self.buttons[-len(self.page_defs):]): b.setChecked(i==index)
        page=self.stack.currentWidget()
        if hasattr(page,'fade_in') and self.settings['page_animation'] and self.animations: page.fade_in()
